...
export const allGenres = (clase: string, lenguage: string | null): AppThunk => {
	return async (dispatch) => {
		try {
			if (clase === "all") {
				const responseMovie = await genres("movie", lenguage);
				const responseTv = await genres("tv", lenguage);
				dispatch(setGenresMovies(responseMovie.data.genres));
				dispatch(setGenresTv(responseTv.data.genres));
			} else if (clase === "movie") {
				const responseMovie = await genres(clase, lenguage);
				dispatch(setGenresMovies(responseMovie.data.genres));
			} else {
				const responseTv = await genres("tv", lenguage);
				dispatch(setGenresTv(responseTv.data.genres));
...
export const { setGenresMovies, setGenresTv } = genreSlice.actions;
